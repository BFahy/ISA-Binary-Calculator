...
# Logic used for division
  ...
  def get_last_calculation(self):
    ...
    last_value = f"The last result: {int(self.history_registers[self.temp_history_index], 2)}"
    self.update_display(last_value)

# Reading binary instructions
  def binary_reader(self, instruction):
    if len(instruction) != 32:
      self.update_display("Invalid instruction length")
      return
    # Splitting instruction into stored variables for performing functions & reading value
